...
def _view(count: int, *, author_id: int = 1) -> LogsPagerView:
    return LogsPagerView(pages=_pages(count), author_id=author_id, embeds=EmbedFactory())
...
class TestJumpToPageModal:
    async def test_valid_page_jumps(self) -> None:
        view = _view(5)
        modal = _JumpToPageModal(view, embeds=EmbedFactory())
        modal.page._value = "3"
        interaction = _interaction(1)
